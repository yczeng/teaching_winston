...
def clicked_square(pos, mouse_pos, image_length):
    x_ok = False
    y_ok = False
    if mouse_pos[0] >= pos[0] - image_length / 4:
        if mouse_pos[0] <= pos[0] + image_length:
            x_ok = True
    if mouse_pos[1] >= pos[1] - image_length / 4:
        if mouse_pos[1] <= pos[1] + image_length:
            y_ok = True
...
def run():
    ...
    while True:
        ...
        if game_over:
            ...
            main_surface.blit(game_over_text, (WINDOW_SIZE//2, WINDOW_SIZE//2))
